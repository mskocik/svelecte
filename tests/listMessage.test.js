...
describe('listMessage', () => {
  ...
  it('empty (fetch)', async () => {
    render(Svelecte, {
      fetch: 'http://localhost:5173/api/not-found',
      fetchDebounceTime: 0,
      lazyDropdown: false
    });
  ...
  it('max (max reached)', async () => {
    ...
    expect(screen.queryByText(i18n.max(2))).toBeInTheDocument();
  });
  it('max + creatabe (max reached)', async () => {
    ...
    expect(screen.queryByText(i18n.max(3))).toBeInTheDocument();
  });
  it('max + creatabe (max reached)', async () => {
    render(Svelecte, {
      options: dataset.colors().slice(0,2),
    ...
    expect(screen.queryByText(i18n.max(4))).toBeInTheDocument();
  });
  it('max + fetch', async () => {
    const { container } = render(Svelecte, {
    ...
    expect(screen.queryByText(i18n.max(1))).toBeInTheDocument();
  });
  // emptyCreatable
  ...
  it('fetchBefore', async () => {
    ...
    expect(screen.queryByText(i18n.fetchBefore)).toBeInTheDocument();
  });
  ...
  it('fetchQuery: multiple item add + remove item', async () => {
    ...
    expect(screen.queryByText(i18n.fetchQuery(2, 0))).toBeInTheDocument();
  ...
  it('fetchEmpty', async () => {
    ...
    expect(screen.queryByText(i18n.fetchEmpty)).toBeInTheDocument();
  });
});
